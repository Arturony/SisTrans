...
import uniandes.isis2304.parranderos.negocio.AdministradorD;
...
class SQLMedico 
{
	 ...
	 */
	public AdministradorD darTipoBebidaPorId (PersistenceManager pm, long idTipoBebida) 
	{
	 ...
	 */
	public List<AdministradorD> darTiposBebidaPorNombre (PersistenceManager pm, String nombreTipoBebida) 
	{
		Query q = pm.newQuery(SQL, "SELECT * FROM " + pp.darTablaTipoBebida  () + " WHERE nombre = ?");
		q.setResultClass(AdministradorD.class);
		q.setParameters(nombreTipoBebida);
		return (List<AdministradorD>) q.executeList();
	}
	 ...
	 */
	public List<AdministradorD> darTiposBebida (PersistenceManager pm)
	{
